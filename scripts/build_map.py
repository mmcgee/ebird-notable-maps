...
#
# Restores per-species layers and LayerControl, so the layer toggle works.
# Popups list ALL checklists for that species at that location, species name on top.
# Info panel aligned above the "i" button. Robust logo loading. Labeled rings. Legend. No MiniMap.
#
...
MAX_RESULTS = 200
ZOOM_START = 11
# If species count exceeds this, fall back to a single combined layer
...
def make_map(lat=CENTER_LAT, lon=CENTER_LON, radius_km=DEFAULT_RADIUS_KM,
    ...
    # Info panel with robust logo src
    ...
    def popup_html_for_entries(species_name: str, loc_name: str, entries: list) -> str:
        ...
        return (
            ...
            f"<div style='font-weight:700;'>{species_name}</div>"
    ...
    if not too_many:
        # One overlay layer per species, with a cluster inside. This enables checkboxes.
        species_groups: dict[str, tuple[folium.FeatureGroup, MarkerCluster]] = {}
        for sp, hexcol in species_to_color.items():
            fg = folium.FeatureGroup(name=sp, show=True, overlay=True, control=True)
        ...
        # Layer control must be added after the FeatureGroups are attached
        folium.LayerControl(collapsed=False).add_to(m)
        add_clear_species_control(m, list(species_to_color.keys()))

    else:
        # Single clustered layer if too many species to keep the control usable
